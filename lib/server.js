...
server.createPhantom = function() {
    ...
    var defaultPhantomArguments = [
        "--load-images=false",
        "--ignore-ssl-errors=true",
        "--ssl-protocol=tlsv1"
    ];
    var phantomArguments = this.options.phantomArguments || defaultPhantomArguments;
    phantomArguments.push({
    ...
    phantomArguments.push(_.bind(this.onPhantomCreate, this));

    phantom.create.apply(this, this.phantomArguments);
};
...
server.onResourceRequested = function (requestData, request) {
    ...
    var abortURIs = this.options.abortURIs || defaultAbortURIs;

    for(var i=0,uri;(uri = abortURIs[i]);i++) {
        if(uri.test(requestData.url)) {
            request.abort();
            break;
        }
    }
